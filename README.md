...
This project covers the end to end implementation of building a ETL batch data pipeline with Dynamic Tableau Dashboard Visualisation using Python and AWS Services for the Capital Markets Stocks Trade Transactions in the various sectors.

The persisted batch stocks data is stored in the AWS S3 Bucket and ingested into the AWS Elastic MapReduce (EMR) Cluster. This ingested data is further transformed using Apache Hive Tables and finally consumed by the Tableau application for displaying the stocks statistics and the summary for the various sectors as a dashboard.  There is also a real-time interface with the Yahoo Charts Spy public URL to retrieve the real-time dynamic stocks status.
